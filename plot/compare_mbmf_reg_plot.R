...
compare_mbmf_reg_plot = function(effect){  
  ...
  plot = ggplot(df, aes(x = drugname, y = effectall,group = drugname)) + 
    geom_half_violin(data = subset(df,drugname == "1"), alpha = 0.5,
                     aes(x = drugname, y = effectall,group = drugname),size = 0.1,
                     fill = "#F8766D",color = "transparent") +
    geom_half_violin(data = subset(df,drugname == "2"), alpha = 0.5,
                     aes(x = drugname, y = effectall,group = drugname),size = 0.1,
                     fill = "#00BFC4",color = "transparent") +
    geom_half_violin(data = subset(df,drugname == "3"), alpha = 0.5,
                     aes(x = drugname, y = effectall,group = drugname),size = 0.1,
                     fill = "#619CFF",color = "transparent") +
    geom_point(size = 0.1,color = "black") + 
    geom_line(aes(group=subname),size = 0.1,color = "gray") +
    stat_summary(data = subset(df,drugname == "1"),aes(x= drugname, y = effectall ),
                 geom="point", shape = 23, color = "red", fill = "red",size = 1.5, alpha = 0.6
    )+
    stat_summary(data = subset(df,drugname == "2"),aes(x= drugname, y = effectall ),
                 geom="point", shape = 23, color = "red", fill = "red",size = 1.5, alpha = 0.6
    )+
    stat_summary(data = subset(df,drugname == "3"),aes(x= drugname, y = effectall ),
                 geom="point", shape = 23, color = "red", fill = "red",size = 1.5, alpha = 0.6
    )+
