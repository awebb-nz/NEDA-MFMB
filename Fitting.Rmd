...
for (dru in 1:ndrug) {
  for (sub in 1:nsub) {
    ...
    tasksub <- taskdrug[
      which(taskdrug$subject == sub & taskdrug$day == drugpos),
    ]
    rewwalk <- cbind(
      tasksub$reward.1, tasksub$reward.2, tasksub$reward.3, tasksub$reward.4
    )
    ...
    optimize <- DEoptim(
      ...
      control = DEoptim.control(
        trace = FALSE, itermax = niter, initialpop = Npop
      )
    )
